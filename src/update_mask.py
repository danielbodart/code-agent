...
def select_top_confidence_positions(masked: torch.Tensor, logits: torch.Tensor, to_unmask: int) -> torch.Tensor:
    """
    Calculate which positions to update based on model confidence.
    
    Args:
        masked: Boolean mask indicating which positions are masked
        logits: Model logits
    ...
    update_mask = torch.zeros_like(masked)
